...
define(['app', 'controllers/HealthModuleViewController'], function (app) {
    app.controller('HealthHomeViewController', ['$scope', 'Ajax', '$location', '$window',
        function ($scope, Ajax, $location, $window) {
            ...
            //切换用户管理
            $scope.currentPatientId = search.patientId;
            Ajax({
                method: 'post',
                data:{
                    idCard : search.idCard
                },
                url: 'ehr/getehrpatientbyidcard',
                suc: function (res) {
                    $scope.userList = res.patientList;
                    $scope.userListMap = {};
                    $.each(res.patientList,function(i,v){
                        $scope.userListMap[v.patientId] = v;
                    })
                }
            });
            //切换用户
            $scope.changeUser = function(user){
                if($scope.currentPatientId != user.patientId){
                    $scope.currentPatientId = user.patientId;
                    $location.search('patientId',user.patientId);
                    //location.reload();
                    YB.healthModuleInit&&YB.healthModuleInit();
                    $scope.userListFlag = false;
                }
            }
            //打开新的页面
            ...
            function getCourseList() {
                Ajax({
                    method: 'post',
                    url: 'ehr/pathogenesis/idcard/list',
                    data: {
                        patientId: _patientId,
                        start: 0,
                        idCard : search.idCard,

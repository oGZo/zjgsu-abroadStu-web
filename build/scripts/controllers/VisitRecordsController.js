...
define(['app','directives/process-add'], function (app,process) {
    app.controller('VisitRecordsModuleViewController', ['$scope', '$http', 'Ajax', '$compile', '$location', '$controller', '$rootScope', '$timeout',
        function ($scope, $http, Ajax, $compile, $location, $controller, $rootScope, $timeout) {
            var scope = $rootScope.$new();
            $http.get('views/visitRecords.html?version='+SYSTEM_VERSION).then(function (res) {
                var dialog = angular.element(res.data);
                $('.J_visit-records').append(dialog);
                var controller = $controller('VisitRecordsController', {
                    $scope: scope,
                    $element: dialog
                }, true, 1);
                $timeout(function () {
    ...
    app.controller('VisitRecordsController', ['$scope', '$cookieStore','Ajax','$location', function ($scope, $cookieStore,Ajax,$location) {
        ...
        //}
        if(search.recordType==2){
            visitRecordInit();
        }
        YB.visitRecordInit = visitRecordInit;
        function visitRecordInit(){
            var pageData = {};

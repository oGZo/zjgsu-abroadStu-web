...
define(['app'], function (app) {
    var moduleApp = {};
    app.controller('ManageClassroomController', ['$scope', '$http', 'Ajax', '$compile', '$location', '$controller', '$rootScope', '$timeout',
        function ($scope, $http, Ajax, $compile, $location, $controller, $rootScope, $timeout) {
            moduleApp.render = function () {
                var scope = $rootScope.$new();
                if(!YB.manageClassroomHtml){
                    $http.get('views/manageClassroom.html?version=' + SYSTEM_VERSION).then(function (res) {
                        YB.manageClassroomHtml = res.data;
                        renderPage();
                    });
                }else{
                    renderPage();
                ...
                function renderPage(){
                    var html = YB.manageClassroomHtml;
                    var dialog = angular.element(html);
                    $('.J_manage_classroom').html(dialog);
                    var controller = $controller('ManageClassroomModuleController', {
                        $scope: scope,
                        $element: dialog
                    }, true, 1);
                    $timeout(function () {
            ...
            if(!YB.manageClassroomHtml&&YB.param.currentManageModuleId==5){
                moduleApp.render();
            }

        }]);
    app.controller('ManageClassroomModuleController', ['$scope','$rootScope', '$cookieStore', '$controller','$compile','$timeout','Ajax','$location','$window',function ($scope, $rootScope,$cookieStore,$controller,$compile,$timeout,Ajax,$location,$window) {
        ...
        var Upload = (function(upload) {
            ...
            upload.submit = function(formNode, callback) {
                ...
                xhr.open('post', window.BASE_URL + 'admin/insertClassroom?' + $.param(urlParam), false);
                xhr.send(formData);
            };
        ...
        //选择图片
        $('#updateClassroomData').on('change', function(event) {
        ...
        //提交表单
        var formNode = document.getElementById('upload-classroom-form'); //图片上传表单节点
        ...
        $scope.pageChanged = function (pageNo) {
            ...
            delete params.num;
            Ajax({
                loadDom :$('.J_classroom_list'),
                method: 'post',
                data: params,
                url: 'admin/selectClassroom',
                suc: function (res) {
                    $scope.bigTotalItems = res.sum;
                    $scope.rows = res.classroomList;
                }
            })
        };
